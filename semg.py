...
# ─────────────────────────────────────────
#  LATENCY SETTINGS  ← tune these
# ─────────────────────────────────────────
# Each buffer adds delay = maxlen / FS seconds
# raw_buf   20 → 100ms  (was 40 → 200ms)
# smooth_buf 3 →  15ms  (was 10 →  50ms)
# graph update every 3 samples → 15ms  (was 10 → 50ms)
RAW_BUF_SIZE    = 20    # RMS window — smaller = faster but noisier
SMOOTH_BUF_SIZE = 3     # smoothing window — smaller = faster response
GRAPH_UPDATE_N  = 3     # update graph every N samples
...
def read_rms():
    """
    Faster RMS: smaller buffer = lower latency.
    raw_buf maxlen=20 → only 100ms of history needed before first reading.
    """
    val = read_raw()
    raw_buf.append(val)
...
def smooth_rms(rms_val):
    """Smaller window = responds faster to changes."""
...
print(f"\n  ✔ Estimated display latency : ~{total_latency_ms:.0f}ms")
...
fig.suptitle(
    f'sEMG Speech Muscle Monitor  |  latency ~{total_latency_ms:.0f}ms',
...
try:
    while True:
        ...
        if graph_update_counter % GRAPH_UPDATE_N == 0 and len(graph_time) > 2:
            ...
            # Dynamic Y scaling — keeps sensitive values visible
            if len(sm_arr) > 0:
